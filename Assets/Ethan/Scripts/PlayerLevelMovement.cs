...
public class PlayerLevelMovement : MonoBehaviour
{
    ...
    private float frozenX; // This is the x position of the player when they are wall running
    private float frozenY; // This is the y position of the player when they are wall running
    ...
    public void OnLeftRight(InputValue value){ // This is a function that is called when the leftRightInput is pressed
        if(isWallRunning){ // If the player is wall running, then return
            return;
        }
    ...
    void FixedUpdate(){// This is a function that is called every fixed delta time
        if(isWallRunning){// If the player is wall running, then check if the jump button is pressed
            ...
            playerP.x = frozenX; // Set the x position of the player to the frozen x position
            playerP.y = frozenY; // Set the y position of the player to the frozen y position
            ...
            vel.z = forwardSpeed; // Set the z velocity of the player to the forward speed
    ...
    void OnCollisionEnter(Collision collision){
        if(!collision.gameObject.CompareTag("WallRunning")){
            return;
        }
        activeWallCollider = collision.collider;
        isWallRunning = true;
        frozenX = playerRigidbody.position.x; // Get the x position of the player by getting the position of the player's rigidbody
        frozenY = playerRigidbody.position.y; // Get the y position of the player by getting the position of the player's rigidbody
        playerRigidbody.useGravity = false; // Set the use gravity of the player's rigidbody to false
    }
    void OnCollisionExit(Collision collision){
        if(!collision.gameObject.CompareTag("WallRunning")){
            return;
        }
        if (activeWallCollider == collision.collider)
